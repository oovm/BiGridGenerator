...
Begin["`Private`"];
ShellSort[start_]:=Module[{liShell,SSort},
  liShell={};liShell=Append[liShell,start];
  SSort[lst_]:=Module[{list=lst,incr,temp,i,j},
  ...
  SSort[start];liShell];
BubbleSort[start_]:=Module[{liBubble,BuSort},
  liBubble={};liBubble=Append[liBubble,start];
  BuSort[array_]:=Module[{arr=array,swapped=False,n=Length[array]},
...
InsertionSort[start_]:=Module[{liInsert,ISort},
  liInsert={};liInsert=Append[liInsert,start];
  ISort[array_]:=Module[{arr=array},
...
CocktailSort[start_]:=Module[{liCocktail,CSort},
  liCocktail={};liCocktail=Append[liCocktail,start];
  CSort[array2_]:=Module[{arr=array2},swapped=True;
  While[swapped==True,swapped:=False;
...
SortShow[startlist_,OptionsPattern[]]:=Module[{n},
...
SortPlot[startlist_,OptionsPattern[]]:=Module[{p},
