...
basicConstraints=Join@@Thread[Unevaluated@constrain[{blocks,rows,cols}]];
problemConstraints=Join@@Table[List/@(Append[k]/@(Position[problem,k]-1)).{81,9,1},{k,1,9}];
allConstraints=(Normal@SparseArray[Thread[#->1],729]&)/@Join[basicConstraints,numberConstraints,problemConstraints];
...
Grid[Partition[FirstPosition[#,1][[1]]&/@Partition[lpResult,9],9],Frame->All]